"""
Utility functions for SlopAuthor.
"""
    ...
    return types.Tool(
        function_declarations=[
            types.FunctionDeclaration(
                name="create_project",
                description="Creates a new project folder in the 'output' directory with a sanitized name. This should be called first before writing any files. Only one project can be active at a time. If a template was specified via command line, it will be automatically applied.",
                parameters=types.Schema(
                    ...
                    properties={
                        ...
                        "template": types.Schema(
                            type=types.Type.STRING,
                            description="Optional template to apply (novel, short_stories, technical_book, screenplay). If not specified, uses the template from command line if any."
                        )
                    },
...
def get_tool_map() -> Dict[str, Callable]:
    ...
    """
    from src.tools import write_file_impl, create_project_impl, compress_context_impl
...
TEMPLATES:
When a user specifies a template (novel, short_stories, technical_book, screenplay), the project will be pre-populated with helpful structure files when you call create_project. These templates provide:
- README.md with project overview
- Outline/structure files
- Character/worldbuilding templates (for fiction)
- Chapter/story templates

If a template is being used:
- The template files will be created automatically when you create the project
- Review the template structure and work within it
- Fill in the template placeholders with actual content
- You can still create additional files as needed
- Follow any template-specific writing guidelines provided
...
2. Create an appropriately named project folder (template files will be auto-created if a template is selected)
3. Review any template files that were created
4. Plan the structure of the work (chapters, stories, etc.)
5. Write COMPLETE, FULL-LENGTH content for each file
6. Update supporting files like README or table of contents as you progress
