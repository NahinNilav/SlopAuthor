#!/usr/bin/env python3
"""
SlopAuthor - An autonomous agent for creative writing tasks.
...
from src.tools.compression import compress_context_impl
from src.tools.templates import list_templates_impl, get_template_manager, get_template_guidelines
from src.tools.project import set_project_template
...
def get_user_input() -> tuple[str, bool, Optional[str]]:
    ...
    Returns:
        Tuple of (prompt/context, is_recovery_mode, template_name)
    """
    parser = argparse.ArgumentParser(
        description="SlopAuthor - Create novels, books, and short stories",
...
Examples:
  ...
  # Use a template
  python writer.py --template novel "Write a mystery novel set in Victorian London"
  
  # List available templates
  python writer.py --list-templates
  ...
  python writer.py --recover my_project/.context_summary_20250107_143022.md
    ...
    parser.add_argument(
        '--template', '-t',
        type=str,
        help='Template to use (novel, short_stories, technical_book, screenplay)'
    )
    parser.add_argument(
        '--list-templates', '-l',
        action='store_true',
    ...
    if args.template:
        ...
        template = manager.get_template_by_name(args.template)
        if not template:
    ...
    if args.recover:
        ...
        return context, True, template_name
    
    # Check if prompt provided as argument
    if args.prompt:
        return args.prompt, False, template_name
    ...
    if not template_name:
        print("\n📚 Available Templates (use --template <name>):")
        manager = get_template_manager()
        for t in manager.list_templates():
            print(f"   • {t.name.lower().replace(' ', '_')} - {t.description}")
    ...
    return prompt, False, template_name


def main():
    """Main agent loop."""
    
    # Check for --list-templates first (doesn't require API key)
    if '--list-templates' in sys.argv or '-l' in sys.argv:
        print(list_templates_impl())
        sys.exit(0)
    
    # Check for --help (doesn't require API key)
    ...
    # Get user input
    user_prompt, is_recovery, template_name = get_user_input()
    
    # Set the project template globally so create_project can use it
    if template_name:
        set_project_template(template_name)
        print(f"📁 Template '{template_name}' will be applied when project is created\n")
    
    # Initialize contents list with raw Content objects
    # This preserves thought_signature and other metadata
    contents: List[types.Content] = []
    
    # Build initial message with template context
    template_context = ""
    if template_name and not is_recovery:
    ...
    if is_recovery:
        ...
        initial_message = user_prompt + template_context
        print(f"\n📝 Task: {user_prompt}\n")
